...
import numpy as np
from models.m1.discriminator import discriminator1
from models.m1.generator import generator1
import os
...
class GAN1():
    ...
    def __init__(self, random_size, roll_size, image_shape, sequence_length, g_optimizer = None, d_optimizer = None):
        ...
        self.g = generator1(g_optimizer, random_size, image_shape, roll_size, sequence_length)
        self.d = discriminator1(d_optimizer, roll_size, image_shape, sequence_length)
    ...
    def build_model(self, g, d):
        ...
        def custom_loss(y_true, y_pred):
            return tf.compat.v1.losses.sigmoid_cross_entropy(y_true, y_pred, label_smoothing=0.05)

        # def accuracy(y_true, y_pred):
        #     return K.sum( K.equal(K.greater(y_pred, K.constant(0.5)),y_true))

        model.compile(loss=custom_loss, metrics=['binary_accuracy'], optimizer=opt)
        # model.compile(loss=custom_loss, metrics=[accuracy], optimizer=opt)
    ...
    def train_step(self, cond_roll_batch, cond_note_batch, img_data_batch, roll_data_batch, BATCH_SIZE, log_g, log_d, verbose):
        ...
        rand_for_notes = np.absolute(np.random.normal(0, 0.01, (roll_data_half_batch.shape)))
        roll_data_half_batch = roll_data_half_batch + rand_for_notes
        ...
        #  Train Discriminator if loss is more than 0.1
        [d_loss_real, d_acc_real] = self.d.evaluate([img_data_half_batch, roll_data_half_batch], valid_half, verbose, HALF_BATCH)
        [d_loss_fake, d_acc_fake] = self.d.evaluate([img_data_half_batch, generated_roll], half_fake, verbose, HALF_BATCH)

        if d_loss_real > 0.1:
            d_real_hist = self.d.train([img_data_half_batch, roll_data_half_batch], valid_half, log_d, verbose, HALF_BATCH)
            d_loss_real = d_real_hist.history['loss'][0]
            d_acc_real = d_real_hist.history['binary_accuracy'][0]

        if d_loss_fake > 0.1:
            d_fake_hist = self.d.train([img_data_half_batch, generated_roll], half_fake, log_d, verbose, HALF_BATCH)
            d_loss_fake = d_fake_hist.history['loss'][0]
            d_acc_fake = d_fake_hist.history['binary_accuracy'][0]

        #  Train Generator
        g_loss = self.train_combined([img_data_batch, noise], valid, log_g, verbose, BATCH_SIZE)
