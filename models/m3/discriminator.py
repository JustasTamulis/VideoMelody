...
class discriminator():
    ...
    def build_model(self, roll_size, image_shape, sequence_length):
        ...
        # --------------------------------------------------
        # Input piano note
        li_piano = layers.Input(shape=(1,roll_size,), name="Pianoroll_input")
        l_piano = layers.Flatten()(li_piano)
        l_piano = layers.Dense(100,activation='relu')(l_piano)
        l_piano = layers.Dense(100,activation='relu')(l_piano)
        # --------------------------------------------------
        # Input conditioned piano roll
        li_cond = layers.Input(shape=(sequence_length, roll_size), name="conditioned_roll")
        l_cond = layers.LSTM(64, input_shape=(sequence_length, roll_size),return_sequences=True)(li_cond)
        l_cond = layers.LSTM(32,return_sequences=True)(l_cond)
        ...
        # --------------------------------------------------
        # merge
        merge = layers.Concatenate()([l_image, l_cond])
        ...
        merge = layers.Concatenate()([l_comb, l_piano])
